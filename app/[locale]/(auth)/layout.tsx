...
export default async function AuthLayout({ children }: LayoutProps<'/[locale]'>) {
  ...
  return (
    <div className="grid min-h-dvh lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
      {/* ---- Marka paneli ---- */}
      <aside className="relative hidden flex-col justify-between overflow-hidden bg-linear-to-br from-[#0c1f3e] via-[#163b70] to-[#245fba] p-10 text-white lg:flex">
        <div
        ...
        <div className="relative z-10 max-w-md">
          ...
          <p className="mt-3 text-sm leading-relaxed text-[#d7e4fb]">{t('brandLead')}</p>

          <ul className="mt-7 space-y-3">
            {points.map((point) => (
              <li key={point} className="flex items-start gap-2.5 text-[13px] text-[#e3ecfb]">
                <svg viewBox="0 0 20 20" className="mt-0.5 size-4 shrink-0 text-[#8fb9ff]" aria-hidden="true">
                  <path
                    fill="currentColor"
        ...
        <dl className="relative z-10 grid grid-cols-3 gap-4 border-t border-white/15 pt-6">
          {figures.map(([label, value]) => (
            <div key={label}>
              <dt className="text-[11px] text-[#b9d2f8]">{label}</dt>
